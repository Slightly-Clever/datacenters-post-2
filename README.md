...
This package contains the code and data behind Post 2 of the blog series on datacenter impacts on wholesale electricity prices. The post walks through how I built a zone×month panel dataset covering 30 electricity pricing zones (22 PJM + 8 ERCOT) from January 2019 through December 2026.
...
The pre-built panel is included, no API keys are needed:
...
You'll need to get two free API keys:
...
Note: The EIA hourly LMP file (`pjm_lmp_da_merged_2020_2025.csv`, ~200MB) is not included in this package due to its size. Download it from the [EIA Open Data portal](https://www.eia.gov/opendata/) (Electricity → Real-Time Operating Grid → Wholesale electricity prices).
